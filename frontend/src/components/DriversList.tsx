...
const DriversList: React.FC<DriversListProps> = ({
  ...
  useEffect(() => {
    // Загружаем данные водителей из API
    fetch('/api/drivers')
  ...
  const getDriverByName = (name: string) => {
    return availableDrivers.find(d => d.full_name === name);
  };
              ...
              <div style={{
                ...
                {driver && (
                  <div style={{
                    ...
                    Смена: {toHHMM(driver.shift_start)} - {toHHMM(driver.shift_end)}
                  </div>
                )}
              </div>
                    ...
                    >
                      ...
                      {availableDrivers.map(driver => (
                        <option key={driver.id} value={driver.full_name}>
                          {driver.full_name.split(' ')[0]} {driver.full_name.split(' ')[1]}
                        </option>
                      ))}
                    </select>
                  </div>
        ...
        <div style={{
          ...
          {availableDrivers.map(driver => (
            <div key={driver.id} style={{ marginBottom: '4px' }}>
              {driver.full_name} • {toHHMM(driver.shift_start)} - {toHHMM(driver.shift_end)}
            </div>
          ))}
        </div>
      </div>
